...
// 负责人：李启波（marcyli）
//============================================================
- ( NSMutableDictionary *)sendPay:(NSString *)name price:(NSString *)price orderId:(NSString *)orderId appid:(NSString *)appid noncestr:(NSString *)noncestr packageValue:(NSString *)packageValue partnerid:(NSString *)partnerid sign:(NSString *)sign timestamp:(NSString *)timestamp prepayid:(NSString *)prepayid orderno:(NSString *)orderno
{
    ...
    [packageParams setObject:orderno forKey:@"attach"]; // 附加参数
    ...
    [packageParams setObject: @"https://api.oomeng.cn"        forKey:@"notify_url"];  //支付结果异步通知
    [packageParams setObject: orderno forKey:@"out_trade_no"];
    [packageParams setObject:[payRequsestHandler getIPAddress]    forKey:@"spbill_create_ip"];
...
- (NSString *)getWeiXinAttach:(NSString *)orderId
{
    int num = arc4random() % 10001;
    NSString *md5 = [WXUtil md5:[NSString stringWithFormat:@"%d",num]];
    return [NSString stringWithFormat:@"%@iii%@",orderId,md5];
}
