...
#import "OneTextInputCell.h"
#import "OneButtonCell.h"
@interface SettingSecCodeViewController (){
...
@implementation SettingSecCodeViewController
...
-(void)goReg{
    ...
    [NSObject postDataWithHost:Server_Host Path:Api_QuickReg Param:param isCache:NO success:^(id json) {
        ...
        if (IntRelay(json[@"state"]) == 1) {
			[AppUtil loginWith:bself.phone andPass:StrRelay(itemSec.value) complete:^{
				[[NSNotificationCenter defaultCenter] postNotificationName:RefreshTable object:nil];
				NSMutableArray *ar = [[NSMutableArray alloc] init];
				for (int i = 0; i < [self.navigationController.viewControllers count]; i++) {
					id ttt = [self.navigationController.viewControllers objectAtIndex:i];
					[ar addObject:ttt];
					if ([ttt isKindOfClass:[NSClassFromString(@"LoginViewController") class]]) {
						break;
					}
				}
				if (ar.count >= 2) {
