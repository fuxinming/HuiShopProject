...
- (void)drawRect:(CGRect)rect
{
    ...
    for (int i = 0; i < self.prizeArr.count; i++) {
        ...
        label.text = self.prizeArr[i][@"goodsName"];
    ...
    if (self.beginPrizeButton) {
		...
		View_Border_Radius(button, button.width/2, 1, [UIColor orangeColor]);
...
- (void)beginPrize
{
	[AppUtil showAlert:@"温馨提示" msg:@"此次抽奖将消耗您10个金币，是否继续？" handle:^(BOOL cancelled, NSInteger buttonIndex) {
		if(buttonIndex == 1){
			[self choujiang];
...
-(void)choujiang{
	...
	[param setObject:StrRelay([BaiduMapService sharedInstance].currentAddress) forKey:@"motionAddress"];
	[param setObject:[NSString stringWithFormat:@"%f",[BaiduMapService sharedInstance].lon] forKey:@"lng"];
	[param setObject:[NSString stringWithFormat:@"%f",[BaiduMapService sharedInstance].lat] forKey:@"lat"];
	[NSObject postDataWithHost:Server_Host Path:Api_buyer_lucky_draw Param:param isCache:NO success:^(id json) {
		if(IntRelay(json[@"state"]) == 1){
			if (bself.canBegin) {
				bself.progressCount = 0;
				bself.prizeIndex = 0;
				bself.rotatCount = bself.prizeArr.count * 13;
				[bself rollingPrize];
			}
		}else{
			Toast(json[@"msg"]);
