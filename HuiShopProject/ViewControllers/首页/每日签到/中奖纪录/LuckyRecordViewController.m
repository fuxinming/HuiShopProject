...
- (void)viewDidLoad {
    [super viewDidLoad];
	self.navBar.title = @"中奖纪录";
	actArray = [[NSArray alloc] initWithObjects:@"全部",@"已兑换",@"未兑换",@"已过期",nil];
	[self tabActSetting];
...
- (void)CreateContentScrollView
{
	...
	_mScrollPageView.createViewAtIndex = ^(int i){
		LuckyContentView *dView1 = [[LuckyContentView alloc]initWithFrame:CGRectMake(i*SCREEN_WIDTH, 0, SCREEN_WIDTH, height) andType:i];
		dView1.tag = 1008+i;
		return dView1;
	};
