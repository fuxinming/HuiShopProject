...
#import "LuckyContentView.h"
#import "EmptyDataCell.h"
#import "RecordeInfoCell.h"
@implementation LuckyContentView

- (id)initWithFrame:(CGRect)frame andType:(int)type{
    self = [super initWithFrame:frame];
    if (self) {
		self.type = type;
        self.formManager[@"RecordeInfoItem"] = @"RecordeInfoCell";
...
- (void)requestWithPage:(int)pageIndex {
	...
	[NSObject getDataWithHost:Server_Host Path:Api_buyer_lucky_list Param:param isCache:NO success:^(id json) {
...
- (NSMutableArray *)createItems:(NSArray *)array
{
	...
	for (int i = 0 ; i < array.count; i ++) {
		...
		RecordeInfoItem*item1 = [[RecordeInfoItem alloc]init];
		item1.userinfo = dict;
