...
@interface FMListView2Controller (){
}

@property (nonatomic,strong) RETableViewSection *section2;
@end

@implementation FMListView2Controller
...
- (void)viewDidLoad
{
    ...
    _section1 = [RETableViewSection sectionWithHeaderTitle:@""];
    _section2 = [RETableViewSection sectionWithHeaderTitle:@""];
    [self.formManager addSection:_section1];
    [self.formManager addSection:_section2];
    [self addHeader];
    [self addFooter];
}
...
- (void)requestFinish:(NSMutableArray *)dataArray
{
    ...
    [self.section2 replaceItemsWithItemsFromArray:itemsArray];
    //刷新列表
    [self requestComplete];
    [self.section2 reloadSectionWithAnimation:UITableViewRowAnimationNone];
...
//    [self.formTable scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:self.section.items.count- 1 inSection:0] atScrollPosition:UITableViewScrollPositionBottom animated:YES];
