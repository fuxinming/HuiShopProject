...
@implementation OrderDetailProductItem
- (instancetype)init
{
    self = [super init];
    if (self) {
...
- (void)cellWillAppear
{
	...
	UILabel*lab1 = [self createLabel:StrRelay(self.item.userinfo[@"goodsName"]) color:COLOR_333 font:Font_Size_13];
	[lab1 sizeToFit];
	lab1.frame = CGRectMake(img1.right +5, 15, lab1.width, 20);
	[self.contentView addSubview:lab1];
	
	UILabel *price1Label = [self createLabel:[NSString stringWithFormat:@"￥%@",StrRelay(self.item.userinfo[@"goodsPrice"])] color:COLOR_RED_ font:Font_Size_13];
	...
	UILabel*lab3 = [self createLabel:[NSString stringWithFormat:@"x%@",StrRelay(self.item.userinfo[@"goodsQty"])] color:COLOR_333 font:Font_Size_13];
	[lab3 sizeToFit];
