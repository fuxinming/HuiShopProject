...
- (void)cellWillAppear
{
	...
	UILabel*nameLab = [self createLabel:StrRelay(self.item.userinfo[@"name"]) color:COLOR_333 font:Font_Size_14];
	nameLab.frame = CGRectMake(icon.right + 5, icon.top+ 10, SCREEN_WIDTH - 10 - 29, 20);
	[self.contentView addSubview:nameLab];
	
	UILabel*chaoshiLab = [self createLabel:StrRelay(self.item.userinfo[@"marketName"]) color:COLOR_999 font:Font_Size_12];
	chaoshiLab.frame = CGRectMake(icon.right + 5, nameLab.bottom, SCREEN_WIDTH - 10 - 29, 15);
	[self.contentView addSubview:chaoshiLab];
	
	UILabel*priceLab = [self createLabel:[NSString stringWithFormat:@"￥ %@",StrRelay(self.item.userinfo[@"retailPrice"])] color:COLOR_RED_ font:Font_Size_14];
